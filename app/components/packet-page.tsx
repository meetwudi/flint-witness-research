...
export function PacketPage({ packet }: { packet: PacketData }) {
  return <main>
    ...
    <section id="experts" className="experts shell"><header className="section-head"><div><span>04</span><p className="kicker">Potential experts</p></div><h2>Ten candidates for further diligence</h2></header><p className="method">Ranked for research priority based on subject-matter fit, relevant publications, public testimony and visible independence-screen burden. The order is preliminary and is not a retention recommendation. Each email button opens an editable draft in the visitor&apos;s mail application; nothing is sent automatically.</p><div className="expert-grid">{packet.experts.map(e=><article key={e.rank} className="expert"><div className="rank">{String(e.rank).padStart(2,"0")}</div><h3>{e.name}</h3><p className="role">{e.role}</p><dl><dt>Technical fit</dt><dd>{e.fit}</dd><dt>Relevant work</dt><dd>{e.work}</dd><dt>Public litigation experience</dt><dd>{e.litigation}</dd><dt>Points for further diligence</dt><dd>{e.weakness}</dd></dl><div className="source-links">{e.links.map(([label,url])=><a key={url} href={url} target="_blank" rel="noreferrer">{label} ↗</a>)}</div><div className="email-action"><a className="email-btn" href={expertMailto(e,packet)}>Draft email to expert</a><a className="email-evidence" href={e.emailSource} target="_blank" rel="noreferrer">{e.email} · public source ↗</a></div></article>)}</div></section>
